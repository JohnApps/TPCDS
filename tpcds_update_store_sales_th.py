# tpcds_update_store_sales_th.py
...
{duckdb.__version__} \
and {sys.argv[0]}, {db_name}, {loop_count} loops, {threads_count} threads")
...
#
verr = duckdb.connect("md:verdb")
verr.begin()
rows = verr.execute("select max(verno)+1 from ver").fetchall()
for row in rows:
    verno = row[0]
stmt = f"insert into ver(verno,pgm) values({verno}, '{sys.argv[0]}')"
verr.execute(stmt)
verr.commit()
verr.close()
