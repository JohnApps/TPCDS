...
if not skip_init:
    for token, database_name in databases:
        ...
        _log.debug(f"Elapsed time: {elapsed_time}")
...
def query_from_thread(pool: DuckDBPool, query: str):
    ...
    rows = res.fetchall()
    _log.debug(f"Number of rows returned for query {query}: {len(rows)}")
    local_con.close()
    elapsed_time = perf_counter() - tstart
    _log.debug(f"Elapsed time for query {query}: {elapsed_time}")
